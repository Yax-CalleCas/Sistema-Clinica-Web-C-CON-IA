...
namespace MediCita.Web.Servicios.Implementacion
{
    public class CitaService : ICitaService   
    {
        ...
        public async Task<Cita?> VerDetalleCitaPaciente(int idCita, int idPaciente)
        {
            Cita? cita = null;

            using (SqlConnection cn = new SqlConnection(cadena))
            using (SqlCommand cmd = new SqlCommand("usp_Paciente_VerDetalleCita", cn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                using (var dr = await cmd.ExecuteReaderAsync())
                {
                    if (await dr.ReadAsync())
                    {
                        cita = new Cita
                        {
                            ...
                            // Nota médica
                            Nota = dr["Nota"] == DBNull.Value ? null : dr["Nota"].ToString()
                        };
                    }
                }
            }
        ...
        public async Task<List<Cita>> ListarCitasDetalleMedico(int idMedico)
        {
            var lista = new List<Cita>();

            using (SqlConnection cn = new SqlConnection(cadena))
            using (SqlCommand cmd = new SqlCommand("usp_Medico_ListarCitasDetalle", cn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@IdMedico", idMedico);
